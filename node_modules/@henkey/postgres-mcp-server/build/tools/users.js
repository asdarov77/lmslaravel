...
function quoteQualifiedIdent(identifier, schema = 'public') {
...
async function executeCreateUser(input, getConnectionString) {
    ...
    const { username, password, superuser, createdb, createrole, login, replication, connectionLimit, validUntil, inherit } = input;
    try {
        const quotedUsername = quoteIdent(username);
        await db.connect(resolvedConnectionString);
        const options = [];
        if (password)
            options.push(`PASSWORD ${quoteLiteral(password)}`);
        if (superuser)
        ...
        if (validUntil)
            options.push(`VALID UNTIL ${quoteLiteral(validUntil)}`);
...
async function executeAlterUser(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
    const { username, password, superuser, createdb, createrole, login, replication, connectionLimit, validUntil, inherit } = input;
    try {
        ...
        if (superuser !== undefined)
            attributes.push(superuser ? 'SUPERUSER' : 'NOSUPERUSER');
        if (createdb !== undefined)
            attributes.push(createdb ? 'CREATEDB' : 'NOCREATEDB');
        if (createrole !== undefined)
            attributes.push(createrole ? 'CREATEROLE' : 'NOCREATEROLE');
        if (login !== undefined)
            attributes.push(login ? 'LOGIN' : 'NOLOGIN');
        if (replication !== undefined)
            attributes.push(replication ? 'REPLICATION' : 'NOREPLICATION');
        if (inherit !== undefined)
            attributes.push(inherit ? 'INHERIT' : 'NOINHERIT');
        if (connectionLimit !== undefined)
            attributes.push(`CONNECTION LIMIT ${connectionLimit}`);
        if (validUntil !== undefined)
            attributes.push(`VALID UNTIL ${quoteLiteral(validUntil)}`);
...
async function executeGrantPermissions(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
export const grantPermissionsTool = {
...
async function executeRevokePermissions(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
export const revokePermissionsTool = {
...
async function executeGetUserPermissions(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
    ...
    `;
        const result = await db.query(permissionsQuery, params);
        return result;
    }
    catch (error) {
...
export const manageUsersTool = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, username, password, superuser, createdb, createrole, login, replication, connectionLimit, validUntil, inherit, ifExists, cascade, permissions, target, targetType, withGrantOption, schema, includeSystemRoles } = args;
        try {
            switch (operation) {
                case 'create': {
                    if (!username) {
