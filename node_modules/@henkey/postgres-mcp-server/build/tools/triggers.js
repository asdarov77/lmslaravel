...
async function executeGetTriggers(input, getConnectionString) {
...
async function executeCreateTrigger(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
async function executeDropTrigger(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
async function executeSetTriggerState(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
export const setTriggerStateTool = {
...
export const manageTriggersTools = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, schema, tableName, triggerName, functionName, timing, events, forEach, when, replace, ifExists, cascade, enable } = args;
        try {
            switch (operation) {
                case 'get': {
