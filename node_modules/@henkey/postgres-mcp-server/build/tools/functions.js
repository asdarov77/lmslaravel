 ...
 */
async function _getFunctions(connectionString, functionName, schema = 'public') {
 ...
 */
async function _createFunction(connectionString, functionName, parameters, returnType, functionBody, options = {}) {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
 ...
 */
async function _dropFunction(connectionString, functionName, parameters, options = {}) {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
        const schema = options.schema || 'public';
 ...
 */
async function _enableRLS(connectionString, tableName, schema = 'public') {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
 ...
 */
async function _disableRLS(connectionString, tableName, schema = 'public') {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
 ...
 */
async function _createRLSPolicy(connectionString, tableName, policyName, using, check, options = {}) {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
        const schema = options.schema || 'public';
 ...
 */
async function _dropRLSPolicy(connectionString, tableName, policyName, options = {}) {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
        const schema = options.schema || 'public';
 ...
 */
async function _editRLSPolicy(connectionString, tableName, policyName, options = {}) {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
        const schema = options.schema || 'public';
 ...
 */
async function _getRLSPolicies(connectionString, tableName, schema = 'public') {
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(connectionString);
        let query = `
...
export const getFunctionsTool = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, functionName, schema } = args;
...
export const createFunctionTool = {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, functionName, parameters, returnType, functionBody, language, volatility, schema, security, replace } = args;
        const resolvedConnString = getConnectionStringVal(connStringArg);
...
export const dropFunctionTool = {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, functionName, parameters, schema, ifExists, cascade } = args;
        const resolvedConnString = getConnectionStringVal(connStringArg);
...
export const editRLSPolicyTool = {
...
export const getRLSPoliciesTool = {
...
export const manageFunctionsTool = {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, functionName, schema, parameters, returnType, functionBody, language, volatility, security, replace, ifExists, cascade } = args;
        const resolvedConnString = getConnectionStringVal(connStringArg);
        let result;
        try {
            switch (operation) {
                ...
                case 'create': {
                    ...
                    if (!returnType)
                        missingFields.push('returnType');
                    if (!functionBody)
                        missingFields.push('functionBody');
                    ...
                    const normalizedParameters = parameters === undefined || parameters === null ? '' :
                        (typeof parameters === 'string' && parameters.trim() === '') ? '' : String(parameters);
                    result = await _createFunction(resolvedConnString, functionName, normalizedParameters, returnType, functionBody, {
                        language,
                        volatility,
                        schema,
                        security,
...
export const manageRLSTool = {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, tableName, schema, policyName, using, check, command, role, replace, roles, ifExists } = args;
        const resolvedConnString = getConnectionStringVal(connStringArg);
        let result;
        try {
            switch (operation) {
