...
async function executeGetSchemaInfo(input, getConnectionString) {
...
async function executeCreateTable(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
async function executeAlterTable(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
            ...
            for (const op of operations) {
                ...
                switch (op.type) {
                    ...
                    case 'alter': {
                        ...
                        const alterActions = [];
                        if (op.dataType)
                            alterActions.push(`TYPE ${op.dataType}`); // May need USING clause for some type changes
...
async function executeGetEnumsInSchema(connectionString, schema = 'public', enumName, getConnectionString) {
    ...
    `;
        ...
        const result = await db.query(query, params.filter(p => p !== undefined));
        return result;
    }
    catch (error) {
...
async function executeCreateEnumInSchema(connectionString, enumName, values, schema = 'public', ifNotExists = false, getConnectionString) {
    const resolvedConnectionString = getConnectionString ? getConnectionString(connectionString) : connectionString;
    const db = DatabaseConnection.getInstance();
    try {
        await db.connect(resolvedConnectionString);
...
export const manageSchemaTools = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, tableName, schema, columns, operations, enumName, values, ifNotExists } = args;
        try {
            switch (operation) {
                case 'get_info': {
                ...
                case 'get_enums': {
                    const result = await executeGetEnumsInSchema(connStringArg || '', schema || 'public', enumName, getConnectionStringVal);
                ...
                case 'create_enum': {
                    ...
                    const result = await executeCreateEnumInSchema(connStringArg || '', enumName, values, schema || 'public', ifNotExists || false, getConnectionStringVal);
