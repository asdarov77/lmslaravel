...
async function executeMonitorDatabase(input, getConnectionString) {
    ...
    try {
        ...
        if (includeQueries) {
            ...
            if (alertThresholds?.longRunningQuerySeconds) {
                ...
                const longRunningQueries = activeQueriesResult.filter(q => q.duration > threshold);
...
export const monitorDatabaseTool = {
...
async function getDatabaseMetrics(db) {
    const dbInfo = await db.queryOne(`SELECT datname as db_name, pg_size_pretty(pg_database_size(current_database())) as db_size, 
            (now() - pg_postmaster_start_time())::text as uptime, 
            xact_commit as committed_tx, xact_rollback as rolled_back_tx 
     FROM pg_stat_database WHERE datname = current_database()`);
    const connInfo = await db.queryOne(`SELECT 
      (SELECT count(*) FROM pg_stat_activity WHERE state = 'active') as active_connections, 
      (SELECT count(*) FROM pg_stat_activity WHERE state = 'idle') as idle_connections, 
      (SELECT count(*) FROM pg_stat_activity) as total_connections, 
      setting as max_connections 
     FROM pg_settings WHERE name = 'max_connections'`);
    const cacheHit = await db.queryOne(`SELECT sum(heap_blks_hit) / (sum(heap_blks_hit) + sum(heap_blks_read)) as cache_hit_ratio 
