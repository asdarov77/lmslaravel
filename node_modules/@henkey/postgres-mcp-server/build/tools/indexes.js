...
async function executeGetIndexes(input, getConnectionString) {
    ...
    try {
      ...
      `;
            ...
            const results = await db.query(statsQuery, params);
            return results;
        }
        const basicQuery = `
      SELECT 
...
async function executeCreateIndex(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
async function executeDropIndex(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
async function executeReindex(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
async function executeAnalyzeIndexUsage(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
    ...
    `;
        ...
        const allIndexes = await db.query(usageQuery, params);
        ...
        HAVING count(*) > 1
      `;
            const duplicateResults = await db.query(duplicateQuery, tableName ? [schema, tableName] : [schema]);
...
async function formatBytes(db, bytes) {
    const result = await db.query('SELECT pg_size_pretty($1::bigint) as formatted', [bytes]);
    return result[0]?.formatted || '0 bytes';
}
export const analyzeIndexUsageTool = {
...
export const manageIndexesTool = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, schema, tableName, indexName, includeStats, columns, unique, concurrent, method, where, ifNotExists, ifExists, cascade, target, type, minSizeBytes, showUnused, showDuplicates } = args;
        try {
            switch (operation) {
                case 'get': {
