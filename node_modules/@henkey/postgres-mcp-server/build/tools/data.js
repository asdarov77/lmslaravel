...
async function executeQuery(input, getConnectionString) {
    ...
    try {
        ...
        switch (operation) {
            ...
            case 'count': {
                ...
                const result = await db.queryOne(countQuery, queryParams, queryOptions);
                return {
                    operation: 'count',
                    rowCount: 1,
            ...
            case 'exists': {
                ...
                const result = await db.queryOne(existsQuery, queryParams, queryOptions);
                return {
                    operation: 'exists',
                    rowCount: 1,
...
export const executeQueryTool = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, query, parameters, limit, timeout } = args;
...
async function executeMutation(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
export const executeMutationTool = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, table, data, where, conflictColumns, returning, schema } = args;
        const resolvedConnString = getConnectionStringVal(connStringArg);
        try {
            // Input validation
            if (!table?.trim()) {
...
async function executeSql(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
export const executeSqlTool = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, sql, parameters, expectRows, timeout, transactional } = args;
        const resolvedConnString = getConnectionStringVal(connStringArg);
        try {
            // Input validation
            if (!sql?.trim()) {
