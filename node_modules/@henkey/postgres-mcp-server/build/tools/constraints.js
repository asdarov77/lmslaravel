...
async function executeGetConstraints(input, getConnectionString) {
...
export const getConstraintsTool = {
...
async function executeCreateForeignKey(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
    const { constraintName, tableName, columnNames, referencedTable, referencedColumns, schema, referencedSchema, onUpdate, onDelete, deferrable, initiallyDeferred } = input;
    try {
        await db.connect(resolvedConnectionString);
...
async function executeDropForeignKey(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
...
async function executeCreateConstraint(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
    const { constraintName, tableName, constraintType, columnNames, checkExpression, schema, deferrable, initiallyDeferred } = input;
    try {
        await db.connect(resolvedConnectionString);
        const schemaPrefix = schema !== 'public' ? `"${schema}".` : '';
        const deferrableClause = deferrable ? ' DEFERRABLE' : '';
...
async function executeDropConstraint(input, getConnectionString) {
    const resolvedConnectionString = getConnectionString(input.connectionString);
    const db = DatabaseConnection.getInstance();
    const { constraintName, tableName, schema, ifExists, cascade } = input;
    try {
...
export const dropConstraintTool = {
...
export const manageConstraintsTool = {
    ...
    execute: async (args, getConnectionStringVal) => {
        const { connectionString: connStringArg, operation, schema, constraintName, tableName, constraintType, columnNames, referencedTable, referencedColumns, referencedSchema, onUpdate, onDelete, constraintTypeCreate, checkExpression, deferrable, initiallyDeferred, ifExists, cascade } = args;
        try {
            switch (operation) {
                case 'get': {
