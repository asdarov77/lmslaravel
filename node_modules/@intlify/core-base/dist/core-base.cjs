    ...
    // for vue-devtools timeline event
    {
        ...
        if (emitter) {
            emitter.emit('missing', {
                ...
                key,
                type,
                groupId: shared.toDevtoolsGroupId(type, key)
            });
        }
    }
    if (missing !== null) {
    ...
    if (shared.inBrowser && emitter) {
        ...
        if (emitter && start) {
            emitter.emit('message-compilation', {
                ...
                message: format,
                time: end - start,
                groupId: shared.toDevtoolsGroupId('translate', key)
            });
        }
        if (startTag && endTag && shared.mark && shared.measure) {
            shared.mark(endTag);
    ...
    if (shared.inBrowser && emitter) {
        ...
        if (emitter && start) {
            emitter.emit('message-evaluation', {
                ...
                value: messaged,
                time: end - start,
                groupId: shared.toDevtoolsGroupId('translate', msg.key)
            });
        }
        if (startTag && endTag && shared.mark && shared.measure) {
            shared.mark(endTag);
